...
#include <vec.h>
	...
	public:
		...
		{
			// Combiner les rotations en multipliant les quaternions
			Quaternion combinedRotation = a.getRotation() * b.getRotation();

			// Appliquer la rotation de 'a' à la translation de 'b' et ajouter la translation de 'a'
			Vector combinedTranslation = a.getTranslation() + a.getRotation() * b.getTranslation();

			return TransformQ(combinedRotation, combinedTranslation);
		}
		...
		{
			Quaternion q = Quaternion::slerp(a.getRotation(), b.getRotation(), (t)); 
			Vector v = a.getTranslation()* (1.0f - t) + b.getTranslation() * t;
			return TransformQ(q, v);
		}
