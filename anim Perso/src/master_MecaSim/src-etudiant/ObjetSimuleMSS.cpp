...
void ObjetSimuleMSS::initMeshObjet()
{
    ...
    /* Creation du Mesh de l objetSimule */
...
{  
    ...
    for(unsigned int i=0; i<_Nb_Sommets; i++)
    {
        ...
        // Modification des coordonnees du sommet
...
void ObjetSimuleMSS::Simulation(Vector gravite, float viscosite, int Tps)
{
    ...
    _SolveurExpl->CalculAccel_ForceGravite(gravite, _Nb_Sommets, A, Force, M, Tps);
    ...
    /* ! Gestion des déchirures */
    Dechirures();

    CollisionSphere(0.5*3, 0.0, -10, 0.0);
...
void ObjetSimuleMSS::Interaction(Vector MousePos){
    ObjetSimule::Interaction(MousePos); 
    if (key_state('p')){
        Chute(); 
    }
}
